...
```text
Admin gọi API
    → Spring Backend (dùng cert User1@org1 do CA ký)
...
Admin approve account
    → registerDID(employeeId, publicKeyJwk)
        ← ghi lên Fabric (cần CA/MSP)

    → issueEmploymentVC(employee)
        ← tạo VC bằng HMAC-SHA256 (độc lập với CA)
...
└─ cấp cert cho User1@org1
    └─ MSP (Org1MSP) nhận cert
        → backend được phép ghi lên Fabric
            └─ RegisterDID
                → lưu publicKeyJwk của employee lên ledger
...
| Thành phần | CA/MSP | Admin cấp VC |
|---|---|---|
